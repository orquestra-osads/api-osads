...
class InventarioController {
	...
	async list(req, res) {
		...
		try {
			...
			Inventario.find({}).select('-updatedAt -__v -createdAt').then((inventario) => {
				return res.json({
					error: false,
					inventario
				});
	...
	async create(req, res) {
		...
		try {
			...
			Inventario.create(bem).then((bem) => {
				return res.json( bem );
	...
	async update(req, res) {
		...
		try {
			...
			Inventario.updateOne({_id: req.params.id}, req.body).then(() => {
				return res.json({
					error: false,
	...
	async delete(req, res) {
		...
		try {
			...
			console.log(bem)
			Inventario.deleteOne({ _id: req.params.id }).then(() => {
				return res.json({
					error: false,
				...
				return res.status(400).json({
					error: true,
					code: 140,
					message: "Error: Bem não foi apagado com sucesso!"
				});
			});
